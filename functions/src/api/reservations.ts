 ...
 *
 * セキュリティ上の意図的なトレードオフ:
 *   メールアドレスは秘密情報ではないため、本エンドポイントは「メールを所持していること」を
 *   証明させない。よって、第三者が会員のメールアドレスを知っていれば本日以降のアクティブ
 *   予約と予約番号を取得でき、その番号で `lookup/cancel` を呼べばキャンセルまで可能になる。
 *
 *   これは確認メール送信を廃止した運用判断（オーナー指示）の必然の結果である。確認メールを
 *   送らないため、予約番号を会員へ事後的に届ける経路はウェブ上の本エンドポイントしかなく、
 *   ここでサマリだけ返すと「番号を控え忘れた会員」が二度と自分でキャンセルできなくなる。
 *   そのため UX 整合性を優先し、予約番号を含む詳細を返す。
 *
 *   不正アクセス耐性は次の層で確保している:
 *   - IP ベースのレートリミット（lookupRateLimit）
 *   - 返却対象を「本日以降のアクティブ予約」に限定（過去・キャンセル済みは漏洩しない）
 *   - キャンセル成否を呼び出し元へ返すのみで、第三者への通知経路は存在しない
 *
 *   将来的に強い本人確認が必要になった場合の選択肢:
 *   - SMS/メールワンタイムコードの導入
 *   - ログイン制（会員アカウント）の導入
 *   - サマリのみ返却＋ハッシュ化メールでの受け取り
 */
